...
interface GoodMoralRecord {
  name: string
}

interface GoodMoralFields {
  semester: string
  schoolYear: string
  isGraduated: boolean
...
interface PrintSnapshot {
  ...
  fields: GoodMoralFields
}

const BLANK_FIELDS: GoodMoralFields = {
  semester: "",
  schoolYear: "",
  isGraduated: false,
  dateGraduated: "",
...
/** "Given this Nth day of Month, year." — matches the legacy frmPrintCOGMC's
 * own ordinal-day + "Month, Year" (prmDay/prmDate) date parameters, computed
 * from a single Date Issued field instead of the legacy's own DateTimePicker. */
function issuedSentence(dateIssued: string): string {
  const [y, m, d] = dateIssued.split("-").map(Number)
  if (!y || !m || !d) return ""
  const monthYear = new Date(y, m - 1, d).toLocaleString("en-US", { month: "long", year: "numeric" })
  return `Given this ${ordinal(d)} day of ${monthYear}, at Manila, Philippines.`
}
...
/**
 * Good Moral / Certificate of Good Moral Character (Registrar > Reports >
 * Student Credentials) — opened as a Dialog from the card on that screen,
 * mirroring Diploma/Honorable Dismissal: create-only (no existing SMS COGMC
 * to load), Print both creates the record and opens the printable
 * certificate in one click.
 *
 * A like-for-like port of the legacy VB system's frmRegCOGMC / frmPrintCOGMC
 * (recovered from the SchoolManagementSystem-ESTI project) — the legacy
 * form used two different Crystal Reports templates depending on whether
 * the student had graduated (rptPrintCOGMC.rpt vs. rptPrintCOGMC2.rpt);
 * this uses a single template with a conditional paragraph instead, same
 * approach as honorable-dismissal.tsx's own graduatedLine. As with that
 * screen, the legacy's exact certificate wording lived as static text baked
 * into a Crystal Reports .rpt binary that isn't recoverable as plain text,
 * so the body copy below is written fresh, carrying the same structured
 * facts the legacy screen bound onto the report: GMC No., Date Issued
 * (as an ordinal-day ceremonial sentence), Checked By, and — only when
 * graduated — the graduation date.
 */
export function GoodMoral({
  open,
  onOpenChange,
  initialStudent,
}: {
  ...
  // same idiom as diploma.tsx/honorable-dismissal.tsx.
  ...
  function set<K extends keyof GoodMoralFields>(key: K, value: GoodMoralFields[K]) {
  ...
  const programEnrollmentQuery = useQuery({
    queryKey: ["Program Enrollment", "good-moral", student?.name],
    queryFn: () =>
  ...
  const programQuery = useQuery({
    queryKey: ["Program", "good-moral", program],
  ...
  const genderQuery = useQuery({
    queryKey: ["Student", "good-moral-gender", student?.name],
  ...
  // "Female" — an unset Gender or any other value falls back to the neutral
  // singular "their" instead of silently guessing, same as diploma.tsx.
  const pronoun =
  ...
  const createMutation = useMutation({
    ...
    mutationFn: (vars: { payload: Record<string, unknown>; snapshot: PrintSnapshot }) =>
      frappe.createDoc<GoodMoralRecord>("SMS COGMC", vars.payload),
    onSuccess: (saved, vars) => {
      toast.success("Certificate of Good Moral Character created")
      queryClient.invalidateQueries({ queryKey: ["SMS COGMC"] })
  ...
  async function openPrintWindow(saved: GoodMoralRecord, snapshot: PrintSnapshot) {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Certificate of Good Moral Character — ${escapeHtml(snapshot.studentName)}</title>
          <style>
        ...
        <body>
          <div class="border">
            ...
            <p class="body-text">
              ...
              ${termLine}, and is known to this Institute to be a person of good moral character.
              ${escapeHtml(titleCaseWord(snapshot.pronoun))} conduct as a student of this Institute has been beyond reproach.
            </p>
            ${graduatedLine}
            <p class="body-text">
              This certification is issued upon the request of the above-named student for whatever legal purpose it may serve.
            </p>
            <p class="given-line">${escapeHtml(issuedSentence(snapshot.fields.dateIssued))}</p>
            <div class="signature">
              <div>${escapeHtml(snapshot.fields.checkedBy)}<br/>Checked By</div>
            </div>
  ...
  function handlePrint() {
    ...
    const payload = {
      ...
      is_graduate: fields.isGraduated ? 1 : 0,
        ...
        <div className="grid gap-5">
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="grid gap-1.5 min-w-0">
              ...
              <StudentSearch selected={student} onSelect={handleStudentSelect} idPrefix="good-moral" />
            </div>
            <div className="grid gap-1.5 min-w-0">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="grid gap-1.5">
                ...
                <Select value={fields.semester} onValueChange={(v) => set("semester", v ?? "")}>
                  <SelectTrigger id="good-moral-semester" className="w-full">
